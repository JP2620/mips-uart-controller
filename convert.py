    ...
    for line in input_lines_list:
        ...
        if(instruction_name in INSTRUCTIONS['R-TYPE'].keys()):
            ...
            if instruction_name in ['SLL', 'SRL', 'SRA']:
                rd = "{:05b}".format(int(operands[0]))   # Registro destino
                rt = "{:05b}".format(int(operands[1]))   # Registro a desplazar
                shamt = "{:05b}".format(int(operands[2])) # Número de bits de desplazamiento
                instruction_bin = op_code + sa + rt + rd + shamt + function
                ...
                instruction_bin = op_code + rs + rt + rd + sa + function
            else:
                rd = "{:05b}".format(int(operands[0]))
